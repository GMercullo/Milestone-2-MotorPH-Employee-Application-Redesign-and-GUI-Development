...
public final class CSVHandler {
    ...
    public static void updateEmployeeInCSV(String filePath, EmployeeData updatedEmp) throws IOException {
        List<EmployeeData> employees = loadAllEmployees(filePath);
        boolean found = false;

        for (int i = 0; i < employees.size(); i++) {
            if (employees.get(i).getEmployeeId() == updatedEmp.getEmployeeId()) {
                employees.set(i, updatedEmp); // Replace existing record
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            System.out.println("Employee ID " + updatedEmp.getEmployeeId() + " not found. No update made.");
            return;
        }

        saveEmployeeToCSV(filePath, employees);
    }
